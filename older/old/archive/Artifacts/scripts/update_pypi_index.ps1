...
{
    Indent
    Write-Host $([string]::Format("Get-Blobs:: Getting blobs from storage '{0}', container '{1}' with prefix '{2}' and suffix '{3}'",$storage.account,$storage.container,$storage.prefix, $suffix))
    ...
    do
    {
        $__Blobs = Get-AzureStorageBlob -Container $storage.container -MaxCount $__MaxReturn  -Prefix $storage.prefix -ContinuationToken $__Token | Where-Object {($_.ICloudBlob.Name.EndsWith($suffix)) -and (-not ($_.ICloudBlob.IsSnapshot)) }
...
function Create-Snapshots($storage, $blobs)
{
    ...
    Indent
    Write-Host $([string]::Format("Create-Snapshots:: Creating snapshots of all {0} blobs in '{1}/{2}/{3}'", $blobs.Length, $storage.account, $storage.container,$storage.prefix))
...
function Create-Backup($storage, $blobs)
{
    ...
    foreach ($_blob in $blobs)
    {
        $__blobname= [string]::Concat($storage.prefix, $__folder, "/", $_blob.ICloudBlob.Name)
...
function Create-Index($target, $storages, $external)
{
    ...
    {
		$__id = $storage.type+"|"+$storage.account+"|"+$storage.resource+"|"+$storage.subscription+"|"+$storage.container+"|"+$storage.prefix
        ...
        {
            $__package_name = $_blob.ICloudBlob.Name.Substring($storage.prefix.Length)
...
{
    Indent
    Write-Host $([string]::Format("Refresh-Index:: Deploying index into '{0}/{1}/{2}'", $storage.account, $storage.container, $storage.prefix))
    ...
    {
        $__blob = [string]::Concat($storage.prefix, $_file.FullName.Substring($path.Length+1))
...
try
{
    ...
    Write-Host "Index update completed"
}
catch
{
